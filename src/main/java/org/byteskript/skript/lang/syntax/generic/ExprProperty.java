...
import org.byteskript.skript.lang.syntax.type.ExprThisThing;
...
public class ExprProperty extends RelationalExpression implements Referent {
    ...
    public ExprProperty() {
        ...
        this.patterns = new java.util.regex.Pattern[]{
                java.util.regex.Pattern.compile("^(?:the )?(?<name>[a-zA-Z0-9 ]+) of (?<input>.+)$"),
                java.util.regex.Pattern.compile("^(?<input>.+)'s (?<name>[a-zA-Z0-9 ]+)$"),
                java.util.regex.Pattern.compile("^(?<input>" + SkriptLangSpec.IDENTIFIER + ")-(?<name>[a-zA-Z0-9 ]+)$")
    ...
    public void compile(Context context, Pattern.Match match) throws Throwable {
        ...
        if (tree != null && context.hasFlag(AreaFlag.IN_TYPE) && tree.current() instanceof ExprThisThing) {
