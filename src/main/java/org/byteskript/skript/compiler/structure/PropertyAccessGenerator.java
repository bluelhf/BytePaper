...
public class PropertyAccessGenerator {
    ...
    public void compile(Context context) throws NoSuchMethodException {
        ...
        for (Map.Entry<Type, Method> entry : handlers.entrySet()) {
            ...
            if (this.type.expectInputs()) {
                ...
                method.writeCode(SyntaxUtils.box(target.getReturnType())); // box if primitive
                method.writeCode(WriteInstruction.cast(cast)); // cast to what the handler needs
            }
            method.writeCode(invoke(target)); // invoke handler
            method.writeCode(SyntaxUtils.box(target.getReturnType())); // box if primitive
        ...
        field_property:
        {
            if (this.type == StandardHandlers.GET) {
                method.writeCode((writer, visitor) -> {
                    visitor.visitVarInsn(25, 0); // load holder
                ...
                method.writeCode(invoke(SyntaxUtils.class.getDeclaredMethod("forceBox", Object.class)));

                method.writeCode((writer, visitor) -> {
                    visitor.visitInsn(176); // areturn
                });
            } else if (this.type == StandardHandlers.SET) {
                method.writeCode((writer, visitor) -> {
                    visitor.visitVarInsn(25, 0); // load holder
                    visitor.visitLdcInsn(name);
